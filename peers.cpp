...
int client::register_request(uint16_t number_of_file,
  ...
  */
  ...
  // connect to server
  if (connect(peer_fd, (struct sockaddr *)&server_address,
  ...
  for (int i = 0; i < number_of_file; i++) {
    ...
    send(peer_fd, &file_infos[i].file_length, sizeof(file_infos[i].file_length),
         0);
  }
...
int client::location_request(std::string file_name, std::vector<chunk> &chunks,
                             int &file_size) {
  /*
  send:
      request type
...
void client::split(const std::string &s, std::vector<std::string> &parameters,
                   const char delim = ' ') {
  parameters.clear();
...
int client::read_log(std::string file_name, std::vector<chunk> &chunks,
                     int file_length) {
  std::ifstream log;
...
int client::download_file(std::string file_name, int index,
  ...
  peer_address.sin_addr.s_addr =
      chunk_instance.peers[selected_peer_index].address.s_addr;
  peer_address.sin_port = htons(chunk_instance.peers[selected_peer_index].port);
  std::cout << "LIST" << std::endl;
  std::cout << "peer: "
            << inet_ntoa(chunk_instance.peers[selected_peer_index].address)
  ...
  // connect to server
  if (connect(sock_fd, (struct sockaddr *)&peer_address, sizeof(peer_address)) <
      0) {
    perror("Failed to connect");
    exit(0);
  }
...
int client::download(std::string file_name) {
  ...
  while (!indexes.empty()) {
    for (int i = 0; i < indexes.size(); i++) {
      ...
      std::thread download_thread(&client::download_file, myclient, file_name,
                                  indexes[i], chunks[indexes[i]]);
  ...
  for (int i = 0; i < chunks.size(); i++) {
    ...
    for (int j = 0; j < chunks[i].peers.size(); j++) {
      std::cout << "ip:" << inet_ntoa(chunks[i].peers[j].address) << ":"
                << chunks[i].peers[j].port << std::endl;
    }
  }
...
void client::handle_request(int process_fd, std::string file_name,
                            int chunk_index) {
  char buffer[CHUNK_SIZE];
...
int client::execute(void) {
  ...
  // bind socket
  if (bind(server_fd, (struct sockaddr *)&peer_address, sizeof(peer_address)) <
      0) {
  ...
  while (1) {
    ...
    if ((process_fd = accept(server_fd, (struct sockaddr *)&client_address,
      ...
      std::cout << "upload file:" << file_name << "," << chunk_index
                << std::endl;
      // use multi-thread to handle requests
      std::thread tid(&client::handle_request, myclient, process_fd, file_name,
                      chunk_index);
      tid.detach();
    }
  }
