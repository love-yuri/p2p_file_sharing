  ...
  for (i = 0; i < files.size(); i++) {
    if (files[i].file_name == file_name) {
      ...
      for (int j = 0; j < number_of_address; j++) {
        send(process_fd, &files[i].peers[j].address,
             sizeof(files[i].peers[j].address), 0);
        send(process_fd, &files[i].peers[j].port,
             sizeof(files[i].peers[j].port), 0);
        ...
        for (int k = 0; k < number_of_chunks; k++) {
          send(process_fd, &files[i].peers[j].chunks[k],
               sizeof(files[i].peers[j].chunks[k]), 0);
        }
      }
    }
  }
...
int server::chunk_register_reply(int process_fd,
  ...
  for (int i = 0; i < temp_files.size(); i++) {
    if (file_name == temp_files[i].file_name) {
      ...
      for (j = 0; j < temp_files[i].peers.size(); j++) {
        if (temp_files[i].peers[j].address == client_address.sin_addr.s_addr &&
            temp_files[i].peers[j].port == port) {
...
void server::handle_request(int new_sock, uint32_t request_type,
                            struct sockaddr_in client_address) {
  switch (request_type) {
  case 0:
...
int server::execute(void) {
  ...
  while (1) {
    ...
    std::thread tid(&server::handle_request, myserver, process_fd, request_type,
                    client_address);
    tid.detach();
